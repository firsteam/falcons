...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update','click_count', 'salenum','(sort_order-is_best*100)'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
	...
	if ($sort=='salenum')
	{
		$sql = 'SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name, g.click_count, g.goods_number, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "LEFT JOIN " . $GLOBALS['ecs']->table('order_goods') .  " as o ON o.goods_id = g.goods_id " . " where g.brand_id = '$brand_id' $cate_where group by g.goods_id ORDER BY $sort $order";
