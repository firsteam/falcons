...
elseif ($_REQUEST['act'] == 'to_shipping')
{
     include(ROOT_PATH.'includes/cls_json.php');
     $json   = new JSON;
     $res    = array('err_msg' => '', 'error_no' => 0);
	 ...
	 if (!empty($invoice_no))
	 {
		  ...
		  {
			  $res['error_no'] = 1;
			  $res['err_msg'] = 'order does not exist';
			  die($json->encode($res));
		  }
		  /* 查询：根据订单是否完成 检查权限 */
		  ...
		  {
			  $res['error_no'] = 1;
			  $res['err_msg'] = 'order_view_finished';
			  die($json->encode($res));
		  }
		  else
		  {
			  /*$res['error_no'] = 1;
			  $res['err_msg'] = 'order_view';
			  die($json->encode($res));*/
		  }
		  ...
		  if(!empty($delivery_stock_result))
		  {
			  foreach ($delivery_stock_result as $value)
			  {
				  if (($value['sums'] > $value['storage'] || $value['storage'] <= 0) && (($_CFG['use_storage'] == '1'  && $_CFG['stock_dec_time'] == SDT_SHIP) || ($_CFG['use_storage'] == '0' && $value['is_real'] == 0)))
				  {
					  ...
					  $res['error_no'] = 1;
					  $res['err_msg'] = sprintf($_LANG['act_good_vacancy'], $value['goods_name']);
					  die($json->encode($res));
		 ...
		 // $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
		  ...
		  $order = order_info($order_id);
		  $res1['order_status'] = $smarty->fetch('order_status.html');
		  $res1['error_no'] = 0;
		  $res1['order_id'] = $order_id;
		  $res1['invoice_no'] = $invoice_no;
		  $res1['err_msg'] = '操作成功';
		 
		  die($json->encode($res1));
	 }
	 else
	 {
		  $res['error_no'] = 1;
