...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$consignee = array(
		...
		'last_name' => empty($address_arr->last_name)  ? '' :   compile_str(trim($address_arr->last_name)),
		...
		'address1' => empty($address_arr->address1)    ? '' :   compile_str($address_arr->address1),
...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	$result['goods_id'] = $goods->goods_id;
	$sql_where = "session_id = '" . SESS_ID . "' ";//添加 www.68ecshop.com
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    // 查看购物车中是否全为免运费商品，若是则把运费赋为零
    //$sql = 'SELECT count(*) FROM ' . $ecs->table('cart') . " WHERE `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
    $sql = 'SELECT count(*) FROM ' . $ecs->table('cart') . " WHERE  `extension_code` != 'package_buy' AND `is_shipping` = 0 AND rec_id in (".$_SESSION['sel_cartgoods'].")"; //jx
    $shipping_count = $db->getOne($sql);
    foreach ($shipping_list AS $key => $val)
    {
...
/* 代码修改_start  By  www.68ecshop.com  将这块替换掉*/
		$sql_where = "a.session_id = '" . SESS_ID . "'";
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = "c.session_id = '" . SESS_ID . "' ";//添加 www.68ecshop.com
/* 代码修改_start  By  www.68ecshop.com  将这块替换掉*/
