...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    if (empty($attribute))
    {
        $link[] = array('href' => 'virtual_goods.php?act=edit&goods_id=' . $goods_id, 'text' => $_LANG['edit_goods']);
...
if($_REQUEST['act']=='verification'){
    ...
    if(!empty($card_sn)){
        ...
        $sql = "select * from ".$ecs->table('virtual_goods_card')." where card_sn = '$card_sn' and supplier_id = $_SESSION[supplier_id]";
        $res = $db -> getRow($sql);
        if($res){
