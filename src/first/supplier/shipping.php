...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_name, shipping_desc, insure, support_cod,shipping_order FROM " .$ecs->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' and supplier_id=".$_SESSION['supplier_id']." ORDER BY shipping_order";
        $row = $db->GetRow($sql);
    ...
    else
    {
        ...
        $sql = "INSERT INTO " . $ecs->table('shipping') . " (" .
                ...
                ",supplier_id) VALUES (" .
                    ...
                    addslashes($_LANG[$modules[0]['desc']]) . "', '$insure', '" . intval($modules[0]['cod']) . "', 1, '" . addslashes($modules[0]['print_bg']) . "', '" . addslashes($modules[0]['config_lable']) . "', '" . $modules[0]['print_model'] . "', $support_pickup,".$_SESSION['supplier_id'].")";
        $db->query($sql);
        $id = $db->insert_Id();
    }
...
elseif ($_REQUEST['act'] == 'uninstall')
{
    ...
    /* 获得该配送方式的ID */
    $row = $db->GetRow("SELECT shipping_id, shipping_name, print_bg FROM " .$ecs->table('shipping'). " WHERE shipping_code='$_GET[code]' and supplier_id=".$_SESSION['supplier_id']);
