...
/* 删除图片 */
else if ($_REQUEST['act'] == 'drop_act_img')
{
    /* 检查权限 */
    admin_priv('favourable');
    ...
    /* 取得logo名称 */
    $sql = "SELECT act_imgfile FROM " .$ecs->table('favourable_activity'). " WHERE act_id = '$act_id'";
    $img_name = $db->getOne($sql);

    if (!empty($img_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/actimg/' .$img_name);
        $sql = "UPDATE " .$ecs->table('favourable_activity'). " SET act_img = 0, act_imgfile = '' WHERE act_id = '$act_id'";
        $db->query($sql);
    }
    $link= array(array('text' => $_LANG['act_edit_lnk'], 'href'=>'favourable.php?act=edit&id=' .$act_id), array('text' => $_LANG['act_list_lnk'], 'href'=>'favourable.php?act=list'));
