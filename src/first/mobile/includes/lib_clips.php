...
{
    $sql = "SELECT c.*, g.goods_name AS cmt_name,g.goods_thumb, r.content AS reply_content, r.add_time AS reply_time ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['goods_thumb'] = '../'.get_image_path($row['id_value'], $row['goods_thumb'], true);
        $comments[] = $row;
    }
...
{
	return $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id = '$user_id'");
}

//获取用户收藏数量
function get_collect_goods_count($user_id)
{
	return $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('collect_goods') . " WHERE user_id = '$user_id'"); 
}

//获取用户评价数量
function get_comment_count($user_id)
{
	return $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('comment') . " WHERE user_id = '$user_id'"); 
}
?>