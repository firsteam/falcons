...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'handler'        => $row['handler'],
					   'goods'			=> get_order_goods($row['order_id']));
    }
    return $arr;
}
...
{
	$sql = "SELECT og.goods_id,og.goods_name,og.goods_price,og.goods_number,g.goods_thumb FROM " .
	$GLOBALS['ecs']->table('order_goods') . " as og , " . 
	$GLOBALS['ecs']->table('goods') . " as g " . 
	"WHERE og.goods_id = g.goods_id AND og.order_id = '$order_id'";
	...
	foreach($list as $key => $val)
	{
		...
		$arr[$key]['goods_price'] = price_format($val['goods_price']);
		$arr[$key]['goods_number'] = $val['goods_number'];
		$arr[$key]['goods_thumb'] = '../'.get_image_path($val['goods_id'], $val['goods_thumb'], true);
	}
	return $arr;
}
