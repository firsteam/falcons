...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'selcart')
{
	...
	if ($_GET['sel_goods'])
	{
		...
		$market_price_desc = $cart_goods['total']['market_price'];
		$res['result'] = "<span class='hot' id='cart_amount_desc'><em>总计:</em>" .$shopping_money."</span>";
		/*
		隐藏市场价格
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	if($flow_type != CART_EXCHANGE_GOODS){
		...
		//验证购物车中提交过来的商品中商品和配件是否正常start
		$sql = "SELECT parent_id,goods_name FROM " . $ecs->table('cart') . " WHERE $sql_where " .
		" AND parent_id > 0 AND rec_id in (" . $_REQUEST['sel_goods'] . ") GROUP BY parent_id";
		...
		if($goods_list)
		{
			...
			for($i = 0; $i < count($goods_list); $i++)
			{
				...
				"AND parent_id = 0 AND rec_id in (" . $_REQUEST['sel_goods'] . ") AND goods_id = '" . $goods_list[$i]['parent_id'] . "'";
				if($db->getOne($sql) == 0)
				{
					$str .= "购物车中".$goods_list[$i]['goods_name']."不符合购买提交，请重新设置或将配件删除<br>";
				}
			}
			if(!empty($str))
			{
				show_message($str,'返回上一页','flow.php');
			}
		}
		unset($sql_where);
		//验证购物车中提交过来的商品中商品和配件是否正常end
    }
...
$sql = "SELECT concat(IFNULL(p.region_name, ''), '  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " .
       ...
       " FROM " . $ecs->table('region') . " p, " . $ecs->table('region') . " t, " . $ecs->table('region') . " d 

   WHERE p.`region_id`='".$consignee['province']."'  AND t.`region_id`='".$consignee['city']."'  AND d.`region_id`='".$consignee['district']."' ";
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
		...
		//yyy新增
		$result['total_price'] = $total['amount_formated'];
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
	...
	$sql_c = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$flow_type' $id_ext";
    $GLOBALS['db']->query($sql_c);
	$_SESSION['sel_cartgoods'] = '';
