...
number = 1;
 ...
 if(document.getElementById(fid)){
  ...
  goods.number   = number;
  goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
  Ajax.call('flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartflowResponse, 'POST', 'JSON');
}
...

{
  ...
  newDiv.className="touchweb_com-indexPop pop_add-cart show";
  ...
  //生成层内内容
	...
	html= '<div class="inner" style="width:290px;"><div class="content_name"><p>' + select_spe + "</p></div>";
	for (var spec = 0; spec < message.length; spec++)
	{
	html += '<div style="text-align:left; background:#ffffff; margin-left:15px;font-weight:600;margin-top: 0px;font-size: 16px; clear:both">' +  message[spec]['name'] + '</div>';
	...
	for (var val_arr = 0; val_arr < message[spec]['values'].length; val_arr++)
	{
          ...
          if (val_arr == 0)
			{
				html+= "<div style='line-height:30px; font-size: 14px;float:left; '><input style='margin-left:15px;' type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' checked /><font color=#555555>" + message[spec]['values'][val_arr]['label'] + '</font> [' + message[spec]['values'][val_arr]['format_price'] + ']</font><br /></div>';
			}
			else
			 {
				html += "<div style='line-height:30px;font-size: 14px;float:left;'><input style='margin-left:15px;' type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' /><font color=#555555>" + message[spec]['values'][val_arr]['label'] + '</font> [' + message[spec]['values'][val_arr]['format_price'] + ']</font><br /></div>';
			}
	 }
...
else
{
		for (var val_arr = 0; val_arr < message[spec]['values'].length; val_arr++)
		{
			html += "<div style='line-height:30px;font-size: 14px;float:left; '><input style='margin-left:15px;' type='checkbox' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' /><font color=#555555>" + message[spec]['values'][val_arr]['label'] + ' [' + message[spec]['values'][val_arr]['format_price'] + ']</font><br /></div>';
		}
		html += "<input type='hidden' name='spec_list' value='" + val_arr + "' />";
		}
}
html += "<div  class='btn_box' style='clear:both;padding-top:20px;'><a href='javascript:cancel_div()' class='btn btn_cancel'>" + is_cancel + "</a><a href='javascript:submit_div(" + goods_id + "," + parent + ")' class='btn btn_confirm'>"+btn_buy+"</a></div>";
html += "</div>";
...

{
...

      {
...

        {
...

          {
            ...
            newDiv.innerHTML += "<div style='line-height:40px; font-size: 14px; float:left;color:#999 '><input style='margin-left:15px;' type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' checked /> <font color=#555555>" + message[spec]['values'][val_arr]['label'] + '</font> [' + message[spec]['values'][val_arr]['format_price'] + ']</font><br /></div>';
...

          {
            ...
            newDiv.innerHTML += "<div style='line-height:40px;font-size: 14px; float:left;color:#999 '><input style='margin-left:15px;' type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' /> <font color=#555555>" + message[spec]['values'][val_arr]['label'] + '</font> [' + message[spec]['values'][val_arr]['format_price'] + ']</font><br /></div>';
  ...
  newDiv.innerHTML += "<div class='cartbntfloat ' ><a href='javascript:submit_div(" + goods_id + "," + parent + ")' class='redBtn' >" + btn_buy + "</a><a href='javascript:cancel_div()' class='greyBtn' >" + is_cancel + "</a></div>";
...
function openSpeDivflow(message, goods_id, parent)
{
  ...
  newDiv.innerHTML += "<div class='cartbntfloat ' ><a href='javascript:submit_divflow(" + goods_id + "," + parent + ")' class='redBtn' >" + btn_buy + "</a><a href='javascript:cancel_div()' class='greyBtn' >" + is_cancel + "</a></div>";
  document.body.appendChild(newDiv);
...
html = '<div class=cardivfloat><div class=cartitle><span class=cartdivfloattitle>商品已成功添加到购物车！</span><a href=\'javascript:cancel_div()\'  class="con_close" style="color: #fff">关闭</a></div></div><div class="cartpopDiv"><div class="toptitle"><a href="goods.php?id='+goods_id+'" class="pic"><img src='+pic+' width="100" height="100"/></a><p><font style="color:#666;font-size:14px;">'+name+'</font>  <font style="color:#ec5151; font-weight:700; font-size:16px;">'+price+'</font></p></div>';

html += '<div class="coninfo">';
html +='购物车共有<font style="color:#ec5151; font-size:14px;"><strong>'+number+'</strong></font>件商品<br>合计：<font style="colorec5151; font-size:14px;"><strong>'+total+'</strong></font>';
html +='</div>'; 
