...
function get_virtual_category(){
    $sql = "select * from ".$GLOBALS['ecs']->table("category")." where parent_id = '0' and is_virtual = '1' and is_show = '1'";
...
function get_virtual_category_chr($id){
    $sql = "select * from ".$GLOBALS['ecs']->table("category")." where parent_id = '$id' and is_virtual = '1' and is_show = '1'";
    $category = $GLOBALS['db'] ->getAll($sql);
    $category_list = array();
  //  $category_list[0]['cat_name'] = '全部';
  //  $category_list[0]['cat_id'] = 0;
