...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'selcart')
{
	...
	{
		$id_ext = " AND rec_id in (". $_REQUEST['sel_goods'] .") ";
		$_SESSION['sel_cartgoods'] = $_REQUEST['sel_goods'];
		...
		/* //折扣活动
		$res['suppid'] = intval($_REQUEST['suppid']);
		$res['your_discount'] = '';
...
if ($_REQUEST['act']=='EditAddress')
{
	...
	if(!empty($_REQUEST['address_id']) 
		&& strtolower($_REQUEST['address_id']) != 'null'
	&& strtolower($_REQUEST['address_id']) != 'undefined')
	{
		$consignee = $GLOBALS['db']->getRow('SELECT * FROM '.$GLOBALS['ecs']->table('user_address').' WHERE address_id='.$_REQUEST['address_id']);
...
elseif ($_REQUEST['act']=='delAddress')
{
	...
	if($_SESSION['flow_consignee']['address_id']==$address_id)
	{
		if ($consignee_list)
		{
			...
			$sql = "update ". $GLOBALS['ecs']->table('users') ." set address_id='". $consignee_list[0]['address_id'] ."' where user_id='".$_SESSION['user_id']."' ";
			$db->query($sql);
...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    if (empty($goods->spec) AND empty($goods->quick))
    {
        ...
        if (!empty($res))
        {
			...
			$result['content'] = $smarty->fetch('goods_attr.lib');
            die($json->encode($result));
        }
    }
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if ($_SESSION['user_id'] == 0)
    {
        ...
        exit; */
		make_json_error('请先登录',NEED_LOGIN_FIRST);
		exit;
    }
    ...
    if($flow_type != CART_EXCHANGE_GOODS){
    	//非积分兑换形式的商品
    	/* 代码增加_start  By  www.68ecshop.com */
		$_SESSION['sel_cartgoods'] =  $_REQUEST['sel_cartgoods'];
		/* 代码增加_end   By  www.68ecshop.com */
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
		...
		/* 代码增加_start  By  www.68ecshop.com */
		$result['supplier_shipping']     = $smarty->fetch('library/order_supplier_shipping.lib');
		/* 代码增加_end   By  www.68ecshop.com */
...
{	
	...
	/* 检查购物车中是否存在赠品，赠品的库存是否充足 */     //jx   2015-1-1 start
