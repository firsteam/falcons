...
if(empty($_SESSION['user_id']))
{
	if(! in_array($action, $not_login_arr))
	{
		...
		{
			make_json_result('', '1秒后跳转到登陆界面', array(
				'need_login' => 1
			));
		}
		else
		{
			...
			make_json_error($_LANG['require_login']);
			exit();
		}
	}
}
...
function action_act_login()
{
	...
	if($user->login($username, $password, isset($_POST['remember'])))
	{
		...
		make_json_result('', $_LANG['login_success'] . $ucdata, array(
			'login_success' => 1
		));
		exit();
...
function action_act_edit_profile()
{
	...
	{
		make_json_error($_LANG['passport_js']['mobile_phone_invalid']);
	}
...
function action_order_list()
{
	...
	include_once (ROOT_PATH . 'includes/lib_transaction.php');
	include_once (APP_ROOT_PATH . 'includes/lib_transaction_2.php');
	...
	$orders = get_user_orders_2($user_id, $pager['size'], $pager['start'], $where);
	...
	$content = $smarty->fetch('order_list.dwt');
	make_json_result($content);
	// $smarty->display('user_transaction.dwt');
}
...
function action_order_detail()
{
	...
	$content = $smarty->fetch('order_details.dwt');
...
function action_EditAddress()
{
	...
	$smarty->assign('country_list', get_regions());
	if(! empty($_REQUEST['address_id']) && strtolower($_REQUEST['address_id']) != 'null' && strtolower($_REQUEST['address_id']) != 'undefined')
	{
