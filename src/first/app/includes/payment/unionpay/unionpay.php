...
*/
$pay_config['SDK_FRONT_TRANS_URL'] = 'https://gateway.95516.com/gateway/api/frontTransReq.do';
$pay_config['SDK_SIGN_CERT_PATH'] = 'includes/modules/payment/unionpay.pfx';
class unionpay

{
	...
	*/
...

	{
		...
		$params = array(
			...
			'frontUrl' =>  !empty($payment['frontUrl']) ? trim($payment['frontUrl']) : return_url(basename(__FILE__, '.php')),  		//前台通知地址

			'backUrl' =>  !empty($payment['backUrl']) ? trim($payment['backUrl']) : return_url(basename(__FILE__, '.php')),		//后台通知地址	
			...
			'merId' => trim($payment['unionpay_account']),//商户代码，请改自己的测试商户号
	...
	*/
...

	{
		...
		//sign_union ($params,$payment['SDK_SIGN_CERT_PATH'],$payment['SDK_SIGN_CERT_PWD']);
		sign_union ($params,SDK_SIGN_CERT_PATH,SDK_SIGN_CERT_PWD);
		file_put_contents('./02.txt',var_export($params,true));
        ...
        {
			file_put_contents('./03.txt','check_money');

            return false;

        }
			file_put_contents('./04.txt',var_export($_POST,true));
			file_put_contents('./05.txt',var_export($payment,true));
...
eot;
    ...
    <input type="submit" type="hidden" value="去支付">
