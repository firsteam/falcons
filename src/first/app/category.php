...
if($act = 'get_goods')
{
	...
	if (!$smarty->is_cached('category.dwt', $cache_id))
	{
		...
		{
			//$page = $max_page;
			make_json_result('','没有更多商品',array('no_more_goods'=>1));
			exit;
		}
	...
	if(!empty($_POST['is_full_page']))
	{
		$smarty->assign('is_full_page',$_POST['is_full_page']);
	}

	$content = $smarty->fetch('goods_list.dwt', $cache_id);
